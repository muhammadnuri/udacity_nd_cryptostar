...
it('can Create a Star', async() => {
    ...
    let actualStar = await instance.tokenIdToStarInfo.call(tokenId)
    assert.equal(actualStar, starName)
});
...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal(await instance.name.call(), 'MN Crypto Star')
    assert.equal(await instance.symbol.call(), 'MNCPTS')
});
...
it('lets 2 users exchange stars', async() => {
    ...
    let tokenId1 = 7
    let tokenId2 = 8
    let star1Name = 'Exchange Test Star 1'
    let star2Name = 'Exchange Test Star 2'
    await instance.createStar(star1Name, tokenId1, {from: accounts[0]})
    await instance.createStar(star2Name, tokenId2, {from: accounts[1]})
...
it('lets a user transfer a star', async() => {
    ...
    let tokenId = 9
    let starName = 'Transfer Test Star'
    await instance.createStar(starName, tokenId, {from: accounts[0]})
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    // 1. create a Star with different tokenId
    let tokenId = 10
    let starName = 'Lookup Star Info Test Star'
    await instance.createStar(starName, tokenId, {from: accounts[0]})
